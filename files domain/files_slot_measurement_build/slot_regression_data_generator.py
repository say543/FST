...
class Data(object):
    ...
    def __init__(self):
        ...
        # <CC> add for user context
        self.patterns_userfilenames = {}  # pattern: userfilenames json
    ...
    def _load_additional_positive_patterns(self, filename, default_freq):
        ...
        with open(filename, encoding='utf-8') as f:
            ...
            for p in pattern_data:
                ...
                #<cc>
                # single quote in json does not work
                #UserFileNamesTemplate = json.loads("{'UserFileNames': ['Liming Ji_Resume_Software Engineer', 'Steve's Team', 'get-installedmodule-output', 'S+A OM focused Shiproom 01282020', 'Email Skill - FY20 H2 Planning', 'Readability in PME', 'Microsoft Search User Job Framework', 'FY21 Substrate Budget Planning', 'Resume_Derek', 'Deprecation Proposal - January 2020 - LT Review', 'SM FY20 preview scenarios', 'Cortana Deprecations Tracker', 'Cortana in Teams R2 Weekly Shiproom', 'Cortana UWP app - skill readiness', 'AI+R Scorecard', 'M365 Search - 3S Instrumentation - Client Contracts', 'Evoke', 'Accept-Decline POV Jan 31-2020', 'FY20 Q3 Email Triage', 'Perf OKRs'], 'UserFileNamesKeyPhrases': ['core skills', 'kingston core skills', 'core skills testing', 'kingston skills', 'kingston device', 'set up cortana', 'kingston core', 'core testing', 'set cortana', 'commute', 'skills testing', 'ios', 'kingston device skills', 'device skills testing', 'kingston device core', 'device core testing', 'device core skills', 'ios commute pass', 'os commute pass', 'device skills']}")
                #UserFileNamesTemplate = json.loads("{\"UserFileNames\": [\"Meeting Notes\", \"Running Experiments\"], \"UserFileNamesKeyPhrases\": [\"core skills\", \"kingston core skills\"]}")
                #give a random filename input
                
                userfilenamestemplate = self.tags['usersfilename'].get_random_value()
                #userfilenamestemplate = json.loads(userfilenamestemplate)
                #print(userfilenamestemplate)


                try:
                    userfilenamestemplate = json.loads(userfilenamestemplate)
                except:
                    raise Exception("wrong file template: {}".format(userfilenamestemplate))
                ...
                #initial UserFileNames
                self.patterns_userfilenames[(p.split('\t')[0])] = userfilenamestemplate
    ...
    def append_attachment_patterns(self, file, newdomain, newintent):
        ...
        with open(file, encoding='utf-8') as f:
            ...
            for p in pattern_data:
                ...
                #initial UserFileNames
                self.patterns_userfilenames[query] = userfilenamestemplate
    ...
    def _get_random_generated_filetag(self, variationSource):
        ...
        # mimic kathan
        # for debug
        #print("generated_filename len: {} ".format(len(random_filename)))
    ...
    def _get_random_expand(self, query, random_tag, prefix=True):
        # remove extra space
        strs = query.strip().split()
        strs_length = len(strs)

        # at most expand 3 tokens
        num_words = random.randint(1, min([strs_length, 3]))
        for i in range(num_words):
            if prefix is True:
                random_tag =  strs[len(strs)-1-i] + ' ' + random_tag
            else:
                random_tag = random_tag + ' ' + strs[i]

        # remove extra space
        return random_tag.strip()

    def _get_mixed_generated_userfilenames(self, query, userfilenames, random_tag):
        
        index = query.find(random_tag)

        if index is -1:
            raise Exception(
                'random_tag {} not exist in {}'.format(random_tag, query))


        # it should have prefix or suffix
        #query_split_by_random_tag = query.split(random_tag)


        prefix = ""
        if index > 0:
            prefix = query[0:index]

        suffix = ""
        if index < len(query)-1:
            suffix = query[index+len(random_tag):len(query)]



        ## has suffix
        generate_new_tag = True
        if index == 0 and index + len(random_tag) != len(query):
            #new_random_tag = self._get_random_expand(query_split_by_random_tag[0], random_tag, prefix=False)
            new_random_tag = self._get_random_expand(suffix, random_tag, prefix=False)
        # has prefix
        elif index + len(random_tag) == len(query) and index !=0:
            #new_random_tag = self._get_random_expand(query_split_by_random_tag[0], random_tag, prefix=True)
            new_random_tag = self._get_random_expand(prefix, random_tag, prefix=True)
        # has both prefix and suffix
        elif index != 0 and index + len(random_tag) != len(query):
            #new_random_tag = self._get_random_expand(query_split_by_random_tag[0], random_tag, prefix=True)
            #new_random_tag = self._get_random_expand(query_split_by_random_tag[1], new_ranodm_tag, prefix=False)
            new_random_tag = self._get_random_expand(prefix, random_tag, prefix=True)
            new_random_tag = self._get_random_expand(suffix, new_random_tag, prefix=False)
        else:
            generate_new_tag = False

        # append new element
        if generate_new_tag is True:
            sub_userfilenames = userfilenames['UserFileNames']
            sub_userfilenames.append(new_random_tag)        


        return userfilenames, generate_new_tag
    ...
    def _get_randomtag_queries(self, pattern, freq):
        ...
        #<cc>
        queries_userfilenames = {} # queries: userfilenames
        ...
        for iter in range(freq):
            ...
            #userfilenames = self.patterns_userfilenames[pattern]
            userfilenames = copy.deepcopy( self.patterns_userfilenames[pattern])

            random_tag_list = []
            random_file_keyword_or_file_name = set([])
            for tag in tags:
...
#                        'Unknown tag {} found in the query pattern {}'.format(tagWoUnderSocre, pattern))
                    ...
                    if tag in ['file_keyword', 'files_keyword'] and filetag_src == 'Additional Tag':
                        ...
                        random_file_keyword_or_file_name.add(random_tag)
                        ...
                        #mimic kathan
                        # <CC> comment below two snince want to generate evaluation data
                        ...
                        random_tag = self.tags['date'].get_random_value()
                        random_tag_list.append(random_tag)
                    elif tag in ['time']:
                        random_tag = self.tags['time'].get_random_value()
                        random_tag_list.append(random_tag)
                    elif tag in ['data_source_name']:
                        # minic file name and get random tag
                    ...
                    elif tag in ['data_source_type']:
                        random_tag = self.tags['data_source_type'].get_random_value()
                        random_tag_list.append(random_tag)  
                    else:
            ...
            # remove random_tag overlapped with file_keyword file_name
            random_tag_list_after_clean = []
            for ele in random_tag_list:
                if ele not in random_file_keyword_or_file_name:
                    random_tag_list_after_clean.append(ele)


            # checking 
            if len(random_tag_list_after_clean) == 0:
                print("no target tag can be replaced, skip generated query: {} from {}".format(query, pattern))
                continue

                #raise Exception(
                #    'random_tag found NONE in the query pattern {}'.format(query))


            # randomly select one span to check
            # version 1
            #random_tag_index_from_list = random.randint(0, len(random_tag_list_after_clean)-1)
            #new_userfilenames, generate_new_tag = self._get_mixed_generated_userfilenames(query, userfilenames, random_tag_list[random_tag_index_from_list])
            #if generate_new_tag is False:
            #    print("no available span identified, skip generated query: {} from {}".format(query, pattern))
            #   continue
            
            # version 2 , only pick pne 
            #random_tag_index_from_list = random.randint(0, len(random_tag_list_after_clean)-1)
            #sub_userfilenames = userfilenames['UserFileNames']
            #sub_userfilenames.append(random_tag_list_after_clean[random_tag_index_from_list])
            #new_userfilenames = userfilenames



            # version 3, pick at least one
            random_tag_num_from_list = random.randint(1, len(random_tag_list_after_clean))
            select_sub_list = random.sample(random_tag_list_after_clean, random_tag_num_from_list)
            sub_userfilenames = userfilenames['UserFileNames']
            for ele in select_sub_list:
                sub_userfilenames.append(ele)
            new_userfilenames = userfilenames
            ...
            # <cc>
            queries_userfilenames[query] = new_userfilenames
        ...
        return queries, queries_domain, queries_annotated_queries, queries_intent,queries_userfilenames
    ...
    def _get_queries_domains_annotated_queries_intent_userfilenames(self, pattern_dict, freq_multiplier):
        queries_domains_annotated_queries_intent_userfilenames = []

        # for each pattern type, replace tags with random tag values
        for pattern, _ in tqdm(pattern_dict.items()):  # Removed freq


            queries, queries_domain, queries_annotated_queries, queries_intent, queries_userfilenames = self._get_randomtag_queries(
                #pattern, freq*freq_multiplier)) # Removed dataset freq to give equal importance
                pattern, freq_multiplier) # Removed dataset freq to give equal importance

            for query in queries:
                queries_domains_annotated_queries_intent_userfilenames.append("{}\t{}\t{}\t{}\t{}".format(query, queries_domain[query], queries_annotated_queries[query], queries_intent[query], queries_userfilenames[query]))

        ##for debug
        print("number ofqueries_domains_annotated_queries_intent_userfilenames: {}".format(
            len(queries_domains_annotated_queries_intent_userfilenames)))


        return queries_domains_annotated_queries_intent_userfilenames
    ...
    def get_data(self, high_freq_multiplier=1, low_freq_multiplier=1):
        ...
        self.positive_data.extend(self._get_queries_domains_annotated_queries_intent_userfilenames(
            self.high_freq_patterns, high_freq_multiplier))
        self.positive_data.extend(self._get_queries_domains_annotated_queries_intent_userfilenames(
            self.low_freq_patterns, low_freq_multiplier))
    ...
    def write_data(self, data_filename, neg_filename, pos_filename, keylist_filename, suffixkeylist_filename,  teams_positve_data_filename):
        ...
        with open(data_filename, 'w', encoding='utf-8') as f:
            # new header
            header = "id\tquery\tintent\tdomain\tQueryXml\tConversationContext\n"
...
#TaskFrameGUID\tSpeechPeopleDisambiguationGrammarMatches\tConversationalContext\n")
            ...
            for query in tqdm(self.positive_data):
                ...
                #f.write("0\t\t{}\tfiles\n".format(query))
                #print('query with userfilenames{}'.format(query))
                ...
                f.write("0\t{}\tfile_search\tfiles\t{}\t{}\n".format((query.split('\t'))[0],(query.split('\t'))[2],(query.split('\t'))[4]))
    ...
    def write_data(self, data_filename1, data_filename2):

        print("Writing all data {} to file: {}/{}".format(len(self.positive_data), data_filename1,data_filename2))
        with open(data_filename1, 'w', encoding='utf-8') as f1, open(data_filename2, 'w', encoding='utf-8') as f2:
            # new header
            header = "id\tquery\tintent\tdomain\tQueryXml\tConversationContext\n"
            ...
            for query in tqdm(self.positive_data):
                if flag == 0:
                    f1.write("0\t{}\tfile_search\tfiles\t{}\t{}\n".format((query.split('\t'))[0],(query.split('\t'))[2],(query.split('\t'))[4]))
                else:
                    f2.write("0\t{}\tfile_search\tfiles\t{}\t{}\n".format((query.split('\t'))[0],(query.split('\t'))[2],(query.split('\t'))[4]))
                flag = (flag +1 ) %2
...
# add more slot for generatin synthetic set
data.load_addtional_patterns('patterns_FILES.txt')
...
data.write_data(data_filename1='files_validating_data.tsv',
                data_filename2='files_testing_data.tsv'
                )
