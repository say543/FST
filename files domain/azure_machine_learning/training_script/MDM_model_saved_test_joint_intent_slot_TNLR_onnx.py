...
for i, row in df.iterrows():
    ...
    # using untokenized string to tokenize again with padding and truncation
...
model = onnx.load("E:\\azure_ml_notebook\\outputs_onnx\\traced_distill_bert_remote.onnx")
...
# save onnx
# Tokenizing input text
text = "open the lu migration ppt"
fast_tokenized_batch : BatchEncoding = fast_tokenizer(text)
fast_tokenized_text :Encoding  =fast_tokenized_batch[0]
fast_tokenized_text_tokens_copy = fast_tokenized_text.tokens
...
dummy_mask = [1] * len(fast_tokenized_text_tokens_copy)
dummy_mask = torch.tensor([dummy_mask]).to(device)
...
torch.onnx.export(model=model,
    ...
    f=output_dir + '/traced_distill_bert.onnx.bin',
