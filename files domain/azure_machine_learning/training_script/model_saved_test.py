...
class DistilBertForSequenceClassificationFilesDomain(DistilBertPreTrainedModel):
    ...
    def forward(
        ...
        # output domain softmax probability for both labels
        #logits = logits.softmax(dim=1)
        #value, label = torch.max(logits.view(-1, self.num_labels), dim=1)
...
output_dir = './domain_model_saved_test/'
import os, argparse
...
model = DistilBertForSequenceClassificationFilesDomain.from_pretrained(output_dir)
# load tokenizer back
# you need to know exact class for each one
...
with torch.no_grad():
    ...
    outputs = model(test_input_tensor, attention_mask=at_mask_tensor)
