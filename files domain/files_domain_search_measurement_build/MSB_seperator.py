...
data_files = glob('./MSBQueriesAll/*.tsv')
...
#
df = pd.read_csv('./msb_nrgative_data/MSBQueriesAll.tsv', sep='\t', encoding="utf-8")

# https://www.geeksforgeeks.org/python-pandas-dataframe-drop_duplicates/

print("df shape {}d".format(df.shape))
...
print("df shape after dedup {}".format(df.shape))
...
while i < len(df.index):
    bound = min(i+offset-1, len(df.index)-1)
    dfs.append(df[i:bound])
    i = bound+1

print("df chunked size {}".format(len(dfs)))

for j in range(0, len(dfs)):
    df = dfs[j]
    filename = './msb_nrgative_data/chunk' +'_'+str(j)+'.tsv'
