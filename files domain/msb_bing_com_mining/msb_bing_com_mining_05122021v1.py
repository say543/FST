...
def get_ngrams(data_files, xgram_para = 2):
    ...
    return xgram_query, xgram_cnt , xgram_file


# function to filter
#def ngrams_compound_heurictic(xgram_cnt_sort_by_cnt ):
#    n_grams = ngrams(word_tokenize(text), n)
#    return [ ' '.join(grams) for grams in n_grams]
...
# for debug
#for key, value in xgram_cnt.items():
#    print("key : {}, value : {}".format(key, value))
'''



xgram_query, xgram_cnt, xgram_file = get_ngrams(data_files = data_files,
                                                xgram_para =2)


# inverse sorting order
#xgram_cnt_sort_by_cnt = dict(sorted(xgram_cnt, key=lambda item: item[1], reverse=True))
...
with open("ngram.tsv", 'w', encoding='utf-8') as fout:
