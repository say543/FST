...
outputFile = 'emailOneShot_slot_training.tsv'
# replace directly
outputTrainingFolderFile = '..\\emailOneShot_slot_training.tsv'
# for STCA test
#outputSTCATrainingFolderFile = '..\\sharemodeltest\\emailOneShot_slot_training.tsv'

outputFileWithSource = "emailOneShot_slot_training_with_source.tsv"
...
for file in files:
    ...
    with codecs.open(file, 'r', 'utf-8') as fin:
        for line in fin:
            ...
            # only extra first five
            if file == 'email_crowdsource_multiturn.tsv' or  file == 'email_synthetic_send_email.tsv':
                line = '\t'.join(array[0:5])
...
#original email data head should be
# UUID	query	intent	domain	slot	sessionId	turnId	weight	timex	metaItem	externalFeature	
