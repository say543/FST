...
fallback_data_files = glob('E:/mdm_data_analysis/web_fallback_profiling/MDM/fallback/*')
web_data_files = glob('E:/mdm_data_analysis/web_fallback_profiling/MDM/web/*')


fallback_df = None
df_list = [] 
for file in fallback_data_files[:]:

    print("file name {}".format(file))


    df = pd.read_csv(file, sep='\t', encoding="utf-8", keep_default_na=False, dtype={
    'query': str, 'score': float})


    print("len before duplication {}".format(len(df)))

    #df = df.sort_values("query", inplace=True)
    #df = df.drop_duplicates(subset ="query", keep=False, inplace=True)
...
web_df = None
df_list = [] 
for file in web_data_files[:]:

    print("file name {}".format(file))


    df = pd.read_csv(file, sep='\t', encoding="utf-8", keep_default_na=False, dtype={
    'query1': str, 'score1': float})
