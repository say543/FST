...
# read calendar golden since most are from calendar golden
#training_queru_before_agumentation = pd.read_csv('./original_parse_data/Mustpass_Default_Calendar_Golden.tsv', sep='\t', encoding="utf-8",
#dtype={'MessageId': object, 'MessageFrom': object, 'MessageText': object, 'ConversationContext': object, 'JudgedDomain': object, 'JudgedConstraints': object})
...
# create dictinoary using MDM hotfix query_with_context as key
aether_dic = {}
with codecs.open('./original_parse_data/aether_hotfix_output.txt', 'r', 'utf-8') as fin:
    for line in fin:
        line = line.strip();
        if not line:
            continue;
        linestrs = line.split("\t");
        aether_dic[linestrs[0].lower()] = line
...
with codecs.open('./original_parse_data/assistant_enus_tvs_uber_mv1.hotfix.txt', 'r', 'utf-8') as fin:
    for line in fin:
        ...
        if linestrs[0].lower() not in aether_dic.keys():
            print('igonre conversation with query: {}'.format(linestrs[0]))
        else:
            Output.append(aether_dic[linestrs[0].lower()])
